...
<body class="min-h-screen flex">
    ...
    <div class="flex flex-1 flex-col justify-center items-center bg-[#f3f7f8] px-6 py-12 md:w-1/2">
        <div class="w-full max-w-sm">
            <h2 class="text-center font-semibold text-lg text-[#1a202c] mb-1">Reset Password</h2>
            <p class="text-center text-xs text-[#4a5568] mb-8">Enter your email and new password</p>

            @if (session('status'))
                <div class="text-green-600 text-xs text-center mb-4">
                    {{ session('status') }}
                </div>
            @endif
